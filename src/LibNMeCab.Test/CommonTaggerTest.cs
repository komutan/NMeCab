...
namespace LibNMeCab.Test
{
    public class CommonTaggerTest : IDisposable
    {
        ...
        public void UnknownSpeech()
        {
            ...
            {
                Assert.True(nodes[i].IsBest);
                Assert.Equal(nodes[i].Prev.EPos, nodes[i].BPos);
                Assert.Equal(nodes[i].Next.BPos, nodes[i].EPos);
                Assert.Equal(nodes[i].EPos - nodes[i].BPos, nodes[i].RLength);
                Assert.Equal(nodes[i].EPos - nodes[i].BPos, nodes[i].Length);
            }
        }
        ...
        public void NBest()
        {
            ...
            foreach (var nodesX in results.Take(5))
            {
                ...
                {
                    Assert.NotEmpty(nodesX[i].Surface);
                    Assert.NotEmpty(nodesX[i].Feature);
                    if (i > 0) Assert.Equal(nodesX[i - 1].EPos, nodesX[i].BPos);
                    if (i + 1 < nodesX.Length) Assert.Equal(nodesX[i + 1].BPos, nodesX[i].EPos);
                    Assert.Equal(nodesX[i].EPos - nodesX[i].BPos, nodesX[i].RLength);
                    Assert.Equal(nodesX[i].EPos - nodesX[i].BPos, nodesX[i].Length);
                }
            }
        }
