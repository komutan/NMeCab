...
NuGetで `LibNMeCab.IpaDicBin` をインストールすると、依存関係にある `LibNMeCab` も同時にインストールされます。
これだけでライブラリと辞書の準備が整い、コーディングを開始できます。
...
- 補足
  ...
  - IPA辞書はやや古い辞書ですが、サイズが小さく、解析結果の精度と一貫性が十分に高く、OSSであるため、最初の使用にお勧めできます。もし異なる辞書を使用したい場合は、次の章を参考にしてください。

### 辞書を自分で用意して使用する

`LibNMeCab` だけをNuGetでインストールし、辞書は自分で用意したものを使用することもできます。

中核部分は言語の文法に依存しない設計なので、日本語以外の言語の辞書も使用可能です。
