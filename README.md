...
プログラムコードのコンパイラのように、文字列を文法ベースで処理していると思われるかもしれませんが、実際にそのような処理を行っても自然言語にはほとんど通用しません。
一般によく用いられる、そしてNMeCabも使っている手法は、事前にコーパス（言語学的にまとめられた大量の資源）から統計的に抽出した辞書情報を使うものです。  
NMeCabの場合、コーパスベースの辞書引き（DoubleArrayTrieを活用）と、規則ベースの未知語抽出により、ラティスと呼ばれる候補の形態素の集合を作り、そこからもっとも確からしい結果を抽出する（最小コスト法を活用）、という流れで解析を行なっています。
